...
namespace oact
{
    ...
    void ActionGroup::addAction(const IActionRef& pAction)
    {
    ...
    void ActionManager::doAction(const IActionRef& pAction)
    {
    ...
    void ActionManager::addAction(const IActionRef& pAction)
    {
        m_history.resize(m_position - m_history.begin());
        m_history.push_back(pAction);
        if (m_maxHistory)
    ...
    {
        doAction(std::make_shared<Action>(name, onRedo, onUndo, onInit, onDestroy));
    }
    ...
    void ActionManager::addAction(const std::string& name,
                                  ...
                                  const Action::ActionCallBack& onDestroy)
    {
        addAction(std::make_shared<Action>(name, onRedo, onUndo, onInit, onDestroy));
    }
    ...
    IActionRef ActionManager::getRedo() const
    {
        if (!canRedo()) return nullptr;
        return *m_position;
    }

    IActionRef ActionManager::getUndo() const
    {
